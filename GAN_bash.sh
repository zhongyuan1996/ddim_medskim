...
for SEED in ${seeds[@]}
        ...
        do
          ...
          if [ DISEASE=="ARF" ]; then
              CUDA_VISIBLE_DEVICES=0, python3 ALL_GAN_runner.py --seed=$SEED --model=$MODAL --target_disease=$DISEASE --max_len=12 --max_num_codes=5132 --n_epochs=30 -bs=128
          if [ DISEASE=="Shock" ]; then
              CUDA_VISIBLE_DEVICES=0, python3 ALL_GAN_runner.py --seed=$SEED --model=$MODAL --target_disease=$DISEASE --max_len=12 --max_num_codes=5795 --n_epochs=30 -bs=128
          if [ DISEASE=="mortality" ]; then
              CUDA_VISIBLE_DEVICES=0, python3 ALL_GAN_runner.py --seed=$SEED --model=$MODAL --target_disease=$DISEASE --max_len=48 --max_num_codes=7727 --n_epochs=30 -bs=128
          else
