...
class Bimodal_AE(nn.Module):
    def __init__(self, seq_len, n_features, ts_embedding_dim=256, tb_embedding_dim=32):
        ...
        self.encoder = ICU_Encoder(seq_len, n_features, ts_embedding_dim=ts_embedding_dim,
                                   tb_embedding_dim=tb_embedding_dim)
...
class HMP(nn.Module):
    def __init__(self, seq_len, n_features, d_model=256, dropout=0.9):
        ...
        self.t5 = AutoModelForSeq2SeqLM.from_pretrained(
            "LLM/physionet.org/files/clinical-t5/1.0.0/Clinical-T5-Base").encoder
...
class Transformer_Encoder(nn.Module):
    def __init__(self, vocab_size1, vocab_size2, d_model=256, dropout=0.1, dropout_emb=0.1, length=48):
        ...
        self.embbedding1 = nn.Sequential(nn.Linear(vocab_size1, d_model), nn.LayerNorm(d_model), nn.ReLU(),
                                         nn.Linear(d_model, d_model), nn.LayerNorm(d_model), nn.ReLU())
        self.embbedding2 = nn.Sequential(nn.Linear(vocab_size2, d_model), nn.LayerNorm(d_model), nn.ReLU(),
                                         nn.Linear(d_model, d_model), nn.LayerNorm(d_model), nn.ReLU())
    ...
    def forward(self, x1, x2, nums_icd, nums_drug, X, S, doc_emb, masked_icd, masked_drug):
        ...
        icu_rep = torch.cat((self.ICU_Encoder(ts_1, tb_1), self.ICU_Encoder(ts_2, tb_2), self.ICU_Encoder(ts_3, tb_3)),
                            -1)
        ...
        input_seqs_mcm_x1 = torch.cat(
            (doc_emb.unsqueeze(1), masked_x1.unsqueeze(1), x2.unsqueeze(1), icu_rep.unsqueeze(1)), 1)
        input_seqs_mcm_x2 = torch.cat(
            (doc_emb.unsqueeze(1), x1.unsqueeze(1), masked_x2.unsqueeze(1), icu_rep.unsqueeze(1)), 1)
