...
class testSimpleRNN(nn.Module):
    ...
    def __init__(self, config, vocab_size, d_model, h_model, dropout, dropout_emb, device):
        ...
        self.lstm = nn.LSTM(d_model, h_model, num_layers=1, batch_first=False)
    ...
    def forward(self, input_seqs, seq_time_step):
        ...
        # print(visit_embedding.shape)
        # exit()
        for i in range(visit_size):
            _, (seq_h, seq_c) = self.lstm(visit_embedding[:, i:i + 1, :].permute(1, 0, 2).clone(),
                                       (seq_h.clone(), seq_c.clone()))

            # try:
            #     _, (seq_h, seq_c) = self.lstm(visit_embedding[:, i:i+1, :].permute(1,0,2).clone(), (seq_h.clone(), seq_c.clone()))
            # except:
            #     _, (seq_h, seq_c) = self.lstm(visit_embedding[:, i:i+1, :].permute(1,0,2).clone())
            # print(seq_h.shape,visit_embedding[:, i:i+1, :].permute(1,0,2).shape)
            # exit()exit
            # seq_h = seq_h.squeeze(0)
            # seq_c = torch.squeeze(seq_c)

            hidden_state_all_visit[:, i:i+1, :] = seq_h.permute(1,0,2)

        # print(hidden_state_all_visit)
        # exit()

        final_prediction = self.classifyer(hidden_state_all_visit[:, -1, :])
        final_prediction_generated = torch.zeros_like(final_prediction)
