...
class HitaNet(nn.Module):
    def __init__(self, vocab_size, d_model, dropout, dropout_emb, num_layers, num_heads, max_pos):
        ...
        self.lowtolarge = torch.nn.Linear(16, 256)
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step, code_masks):
        ...
        input_seqs = self.lowtolarge(input_seqs)
        x = input_seqs + self.bias_embedding
...
class LSAN(nn.Module):
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step, code_masks):
        ...
        input_embedding = self.lowtolarge(input_seqs).unsqueeze(2)
...
class LSTM_encoder(nn.Module):
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step, code_masks):
        ...
        x = self.lowtolarge(input_seqs)
        # b = self.embbedding(input_seqs).sum(dim=2)
        x = self.emb_dropout(x)
...
class Timeline(nn.Module):
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step, code_masks):
        input_seqs = input_seqs.unsqueeze(2)
...
class SAND(nn.Module):
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step, code_masks):
        ...
        x = self.lowtolarge(input_seqs) + self.bias_embedding
        bs, sl, dm = x.size()
        # x = self.emb_dropout(x)
        output_pos, ind_pos = self.pos_emb(lengths)
        x += output_pos
...
class RetainEx(nn.Module):
    def __init__(self, vocab_size, d_model, dropout, dropout_emb, num_layers, num_heads, max_pos):
        super().__init__()
        self.lowtolarge1 = torch.nn.Linear(16, 256)
        self.lowtolarge2 = torch.nn.Linear(16, 256)
    ...
    def forward(self, input_seqs, masks, lengths, time_step, code_masks):
        embedded = self.lowtolarge1(input_seqs)
        embedded2 = self.lowtolarge2(input_seqs)
