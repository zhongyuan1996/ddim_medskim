...
def eval_metric(eval_set, model, encoder):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(eval_set):
            labels, ehr_codes, mask_ehr, time_step, visit_timegap, ehr_codes_timegaps, code_mask, lengths = data
            # labels, ehr, mask, txt, mask_txt, lengths, time_step, code_mask = data
            if encoder == 'adacare':
                logits = model(ehr_codes, device)
            else:
                logits = model(ehr_codes, mask_ehr, lengths, time_step, code_mask)
...
def main(seed, name, model, data, mod, max_len, max_num, sav_dir, mode, short_ICD, toy):
    ...
    parser.add_argument('--modality', default=mod)
...
def train(args):
    ...
    results_file_name = str(args.save_dir) + str(args.name)+ '_' + str(args.model) + '_' + str(args.target_disease) + '_' + str(args.modality)+ '_' + str(args.seed) + '.csv'
    ...
    if args.modality == 'diag':
        pad_id = diag_pad_id
        nan_id = diag_pad_id
    elif args.modality == 'drug':
        pad_id = drug_pad_id
        nan_id = drug_nan_id
    elif args.modality == 'lab':
        pad_id = lab_pad_id
        nan_id = lab_nan_id
    elif args.modality == 'proc':
        pad_id = proc_pad_id
        nan_id = proc_nan_id
    else:
    ...
    if args.short_ICD:
        train_dataset = pancreas_Gendataset_unimodal(data_path + 'train_3dig' + str(args.target_disease)+ '3_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
        val_dataset = pancreas_Gendataset_unimodal(data_path + 'val_3dig' + str(args.target_disease)+ '3_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
        test_dataset = pancreas_Gendataset_unimodal(data_path + 'test_3dig' + str(args.target_disease)+ '3_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
        if args.model != 'none':
            synthetic_dataset = pancreas_Gendataset_unimodal_for_baselines(data_path + str(args.model) + '_synthetic_3dig' + str(args.target_disease) + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
    # elif args.toy:
    #     train_dataset = pancreas_Gendataset_unimodal(data_path + 'toy_3dig' + str(args.target_disease)+ '3_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
    #     val_dataset = pancreas_Gendataset_unimodal(data_path + 'toy_3dig' + str(args.target_disease)+ '3_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
    #     test_dataset = pancreas_Gendataset_unimodal(data_path + 'toy_3dig' + str(args.target_disease)+ '3_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
    #     if args.model != 'none':
    #         synthetic_dataset = pancreas_Gendataset_unimodal_for_baselines(data_path + str(args.model) + '_synthetic_3dig' + str(args.target_disease) + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
    else:
        train_dataset = pancreas_Gendataset_unimodal(data_path + 'train_5dig' + str(args.target_disease)+ '5_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
        val_dataset = pancreas_Gendataset_unimodal(data_path + 'val_5dig' + str(args.target_disease)+ '5_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
        test_dataset = pancreas_Gendataset_unimodal(data_path + 'test_5dig' + str(args.target_disease)+ '5_subset' + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
        if args.model != 'none':
            synthetic_dataset = pancreas_Gendataset_unimodal_for_baselines(data_path + str(args.model) + '_synthetic_5dig' + str(args.target_disease) + '.csv', args.max_len, args.max_num_codes, pad_id, nan_id, args.modality, device)
    train_dataloader = DataLoader(train_dataset, args.batch_size, shuffle=True, collate_fn=gen_collate_fn_unimodal)
    dev_dataloader = DataLoader(val_dataset, args.batch_size, shuffle=False, collate_fn=gen_collate_fn_unimodal)
    test_dataloader = DataLoader(test_dataset, args.batch_size, shuffle=False, collate_fn=gen_collate_fn_unimodal)
    if args.model != 'none':
        synthetic_dataloader = DataLoader(synthetic_dataset, args.batch_size, shuffle=True, collate_fn=gen_collate_fn_unimodal_for_baselines)
    ...
    if args.name == 'hita':
        ...
        model = LSTM_encoder(pad_id, args.d_model, args.dropout, args.dropout_emb, args.num_layers, args.num_heads,
                             args.max_len)
    elif args.name == 'gruself':
        model = GRUSelf(pad_id, args.d_model, args.dropout, args.dropout_emb, args.num_layers, args.num_heads,
                        args.max_len)
    elif args.name == 'retain':
        model = Retain(pad_id, args.d_model, args.dropout, args.dropout_emb, args.num_layers, args.num_heads,
                       args.max_len)
    elif args.name == 'adacare':
        model = AdaCare(pad_id, hidden_dim=args.d_model, kernel_size=2, kernel_num=64, input_dim=args.d_model,
                        output_dim=1, dropout=args.dropout, r_v=4,
    ...
    if args.model != 'none':
        for epoch in range(3):
            for i, data in enumerate(tqdm(synthetic_dataloader, desc="Training with synthetic data")):
                labels, ehr_codes, mask_ehr, time_step, code_mask, lengths = data
                optim.zero_grad()
                if args.name == 'adacare':
                    outputs = model(ehr_codes, device)
                else:
                    outputs = model(ehr_codes, mask_ehr, lengths, time_step, code_mask)
            ...
            for baseline_name in baseline_names:
                for model_name, save_dir in zip(model_names, save_dirs):
                    for data, max_len, max_num in zip(datas, max_lens, max_nums):
                        for mod in modality:
                                main(seed, baseline_name, model_name, data, mod, max_len, max_num, save_dir, mode, short_ICD, toy)