...
from models.ab_LSTM_ddim import *
import yaml
...
def main(name, seed, data, max_len, max_num, lambdaD, lambdaS, AS):
    parser = argparse.ArgumentParser()
    parser.add_argument('--AS', default=AS, type=str, choices=['None', 'AS1', 'AS2', 'AS3'])
...
def train(args):
    ...
    if  str(args.AS) +'_'+str(args.model) + '_LD'+str(args.lambda_DF_loss)+'_LS'+str(args.lambda_CE_gen_loss) + '_' + str(args.target_disease) + '_' + str(args.seed) + '.csv' in files:
        ...
        model = RNNdiff(config, vocab_size=pad_id, d_model=args.d_model, h_model=args.h_model,
                        dropout=args.dropout, dropout_emb=args.dropout_emb, device = device, channel=args.max_len, initial_d=initial_d, AS = args.AS)
        ...
        results_file = open(str(args.save_dir)+ str(args.AS) +'_' + str(args.model) + '_LD'+str(args.lambda_DF_loss)+'_LS'+str(args.lambda_CE_gen_loss) + '_' + str(args.target_disease) + '_' + str(args.seed) + '.csv', 'w', encoding='gbk')
                ...
                for lD in lambdaD:
                    for lS in lambdaS:
                        for data, max_len, max_num in zip(dataset, max_lens, max_nums):
                            main(name, seed, data, max_len, max_num, lD, lS, AS)
